...
// ================================================================
// List-discovery mode (listUrl + fetchList)
// ================================================================

describe("POST /api/v1/scraper/trigger — list-discovery mode", () => {
	it("adapter 有 fetchList → 随机选一条 URL 并成功返回 200", async () => {
		process.env.LLM_ENDPOINT = "https://api.openai.com";
		process.env.LLM_API_KEY = "test-key";

		const listAdapter = {
			name: `list-adapter-${testId}`,
			fetchContent: vi.fn(async () => MOCK_RAW),
			fetchList: vi.fn(async () => [
				"https://test-site.example.com/article/100",
				"https://test-site.example.com/article/101",
			]),
		};
		scraperConfig.registerAdapter(listAdapter);
		scraperConfig.addSiteConfig({
			siteName: `list-site-${testId}`,
		...
		const res = await app.inject({
			method: "POST",
			url: "/api/v1/scraper/trigger",
			payload: { siteName: `list-site-${testId}` },
		});
		expect(res.statusCode).toBe(200);
		expect(res.json().ok).toBe(true);
	...
	it("fetchList 返回空列表 → 404", async () => {
		process.env.LLM_ENDPOINT = "https://api.openai.com";
		process.env.LLM_API_KEY = "test-key";

		const emptyAdapter = {
			name: `empty-adapter-${testId}`,
			fetchContent: vi.fn(async () => MOCK_RAW),
			fetchList: vi.fn(async () => [] as string[]),
		};
		scraperConfig.registerAdapter(emptyAdapter);
		scraperConfig.addSiteConfig({
			siteName: `empty-list-site-${testId}`,
			adapterName: `empty-adapter-${testId}`,
			url: "https://test-site.example.com",
			listUrl: "https://test-site.example.com/list",
			cron: "0 * * * *",
			enabled: true,
		...
		const res = await app.inject({
			method: "POST",
			url: "/api/v1/scraper/trigger",
			payload: { siteName: `empty-list-site-${testId}` },
		});
		expect(res.statusCode).toBe(404);
		expect(res.json().error).toMatch(/No articles found/i);

		delete process.env.LLM_ENDPOINT;
		delete process.env.LLM_API_KEY;
	});

	it("fetchList 抛出异常 → 500", async () => {
		process.env.LLM_ENDPOINT = "https://api.openai.com";
		process.env.LLM_API_KEY = "test-key";

		const failAdapter = {
			name: `fail-list-adapter-${testId}`,
			fetchContent: vi.fn(async () => MOCK_RAW),
			fetchList: vi.fn(async () => {
		...
		scraperConfig.addSiteConfig({
			siteName: `fail-list-site-${testId}`,
			adapterName: `fail-list-adapter-${testId}`,
			url: "https://test-site.example.com",
			listUrl: "https://test-site.example.com/list",
			cron: "0 * * * *",
			enabled: true,
		...
		const res = await app.inject({
			method: "POST",
			url: "/api/v1/scraper/trigger",
			payload: { siteName: `fail-list-site-${testId}` },
		});
		expect(res.statusCode).toBe(500);
		expect(res.json().error).toMatch(/Failed to fetch list/i);
...
// ================================================================
// ENRICHMENT_ENABLED path
// ================================================================

describe("POST /api/v1/scraper/trigger — enrichment enabled", () => {
	it("ENRICHMENT_ENABLED 不为 'false' → 走 enrichContext 分支，仍返回 200", async () => {
		process.env.LLM_ENDPOINT = "https://api.openai.com";
		process.env.LLM_API_KEY = "test-key";
		// 不设置 ENRICHMENT_ENABLED → 默认走 enrichContext
		...
		delete process.env.LLM_ENDPOINT;
		delete process.env.LLM_API_KEY;
		process.env.ENRICHMENT_ENABLED = "false"; // restore
	});
});
...
vi.mock("../scraper/auto-generate.js", () => ({
	autoGenerateDrafts: vi.fn(async () => ({
		generated: 2,
		skipped: 1,
		errors: 0,
	})),
...
describe("POST /api/v1/scraper/auto-generate", () => {
	it("Happy: LLM env set → 200 + progress 字段", async () => {
		process.env.LLM_ENDPOINT = "https://api.openai.com";
		process.env.LLM_API_KEY = "test-key";
		...
		expect(res.statusCode).toBe(200);
		expect(res.json().ok).toBe(true);
		expect(res.json().progress).toBeDefined();
		expect(res.json().progress.total).toBe(3); // 2+1+0

		delete process.env.LLM_ENDPOINT;
		delete process.env.LLM_API_KEY;
	});

	it("LLM env 未设置 → 500", async () => {
		delete process.env.LLM_ENDPOINT;
		delete process.env.LLM_API_KEY;
		...
		expect(res.statusCode).toBe(500);
		expect(res.json().error).toMatch(/LLM_ENDPOINT/i);
	});
});
