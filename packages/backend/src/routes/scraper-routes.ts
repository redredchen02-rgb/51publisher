...
import { type EnrichedContext, enrichContext } from "../scraper/web-enricher.js";
...
export async function registerScraperRoutes(
	...
	app.get("/api/v1/scraper/adapters", async () => {
		const adapters = scraperConfig.listAdapters().map((a: { name: string }) => ({
			name: a.name,
		}));
	...
	app.post<{
		...
		async (request, reply) => {
			...
			try {
				const { autoGenerateDrafts } = await import("../scraper/auto-generate.js");
