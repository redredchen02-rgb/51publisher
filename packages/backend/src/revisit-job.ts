...
import { loadSSRFAllowlist, isHostAllowed } from './scraper/ssrf-allowlist.js';
...
async function checkRow(row: PublishedRow, deps: RevisitDeps): Promise<void> {
  ...
  if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
    deps.logger?.warn(`[revisit] Skipping non-http(s) URL for post ${id}: ${publish_url}`);
    return;
  }

  // SSRF allowlist check — revisit-specific allowlist separate from scraper's
  const config = loadSSRFAllowlist({ ALLOWED_HOSTS: process.env.REVISIT_ALLOWED_HOSTS });
  if (!isHostAllowed(parsed, config)) {
    deps.logger?.warn(`[revisit] Host not in REVISIT_ALLOWED_HOSTS, skipping post ${id}`);
    return;
  }
  ...
  await pendingWriteQueue.enqueue(() => {
    const db = getDb();
    db.prepare('UPDATE published_posts SET outcome = ?, last_checked_at = ? WHERE id = ?').run(outcome, now, id);
  });
}
...
async function runImmediateSweep(deps: RevisitDeps): Promise<void> {
  try {
    ...
    const rows = db
      .prepare('SELECT * FROM published_posts WHERE outcome IS NULL AND created_at > ?')
...
async function runHealthSweep(deps: RevisitDeps): Promise<void> {
  try {
    const db = getDb();
    const rows = db.prepare("SELECT * FROM published_posts WHERE outcome = 'online'").all() as PublishedRow[];
    for (const row of rows) {
      await checkRow(row, deps);
    }
  } catch (err) {
...
export function startRevisitJob(deps: RevisitDeps = {}): void {
  ...
  const immediateTask = cron.schedule(immediateCron, () => runImmediateSweep(deps));
