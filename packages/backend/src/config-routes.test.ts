...
describe('GET /api/v1/config/mappings', () => {
  it('returns default field mapping', async () => {
    ...
    const res = await app.inject({ method: 'GET', url: '/api/v1/config/mappings' });
    expect(res.statusCode).toBe(200);
    const body = res.json();
    expect(body.ok).toBe(true);
    expect(body.mappings).toBeDefined();
    expect(body.mappings.title.selector).toBe(DEFAULT_FIELD_MAPPING.title!.selector);
...
describe('PUT /api/v1/config/mappings', () => {
  it('updates mappings with valid payload', async () => {
    const app = await buildApp();
    const newMappings = {
      title: { selector: 'input#new-title', fieldType: 'text', label: '新標題' },
    };
    ...
    // 验证 GET 也返回更新后的映射
    const getRes = await app.inject({ method: 'GET', url: '/api/v1/config/mappings' });
