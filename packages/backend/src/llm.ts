...
export async function generateDraft(prompt: string, deps: LlmDeps): Promise<GenerateDraftResponse> {
  ...
  for (const currentModel of modelsToTry) {
    ...
    for (const useSchema of [true, false]) {
      const { url, init } = buildRequest(prompt, { ...settings, model: currentModel }, apiKey, {
...
export function extractUsage(raw: unknown): { prompt: number; completion: number } | undefined {
...
export function buildReviewPrompt(draft: ContentDraft, criteriaPrompt?: string): string {
...
export function buildRewritePrompt(draft: ContentDraft, failedDims: string[]): string {
...
export async function reviewDraftLlm(
  ...
  if (!apiKey || !settings.endpoint) return { ok: false, error: '未配置 API key 或端点。' };

  const prompt = buildReviewPrompt(draft, criteriaPrompt);
  const { url, init } = buildRequest(prompt, settings, apiKey, { jsonSchema: false });
  ...
  try {
    ...
    return { ok: false, error: err instanceof Error && err.name === 'AbortError' ? '评审请求超时。' : '网络错误。' };
  } finally {
    clearTimeout(timer);
  }
  ...
  if (!Array.isArray(dims)) return { ok: false, error: '评审结果缺少 dimensions 字段。' };
  ...
  return { ok: true, result: { ok: true, dimensions }, ...(reviewCostTokens ? { reviewCostTokens } : {}) };
}
...
export async function rewriteDraftLlm(
  ...
  try {
    ...
    return { ok: false, error: err instanceof Error && err.name === 'AbortError' ? '重写请求超时。' : '网络错误。' };
  } finally {
    clearTimeout(timer);
  }
  ...
  if (typeof parsed.title === 'string' && parsed.title.trim()) rewritten.title = parsed.title.trim();
  if (typeof parsed.body === 'string' && parsed.body.trim()) rewritten.body = parsed.body.trim();
  if (Array.isArray(parsed.tags)) {
    rewritten.tags = parsed.tags.map((t) => String(t)).filter(Boolean);
  }

  const rewriteCostTokens = extractUsage(raw);
  return { ok: true, draft: rewritten, ...(rewriteCostTokens ? { rewriteCostTokens } : {}) };
}
