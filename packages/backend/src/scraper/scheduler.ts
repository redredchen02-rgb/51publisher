...
import type { SiteAdapter, ScraperSiteConfig } from './site-adapter.js';
...
/** 单条 URL 模式（现有行为）。 */
async function runSingleUrl(site: ScraperSiteConfig, adapter: SiteAdapter, deps: SchedulerDeps): Promise<void> {
  const runStart = Date.now();
  deps.logger?.info(`[scheduler] Triggering ${site.siteName}: ${site.url}`);

  const rawContent = await fetchWithRetry(adapter, site.url, deps, site.siteName);
...
async function runListDiscovery(site: ScraperSiteConfig, adapter: SiteAdapter, deps: SchedulerDeps): Promise<void> {
  const budget = Math.max(1, Number(process.env.ACGS51_LIST_BUDGET ?? '20') || 20);
  deps.logger?.info(`[scheduler] List-discovery start: ${site.siteName} listUrl=${site.listUrl}`);
  ...
  if (freshUrls.length > budget) {
    deps.logger?.warn(`[scheduler] Budget cap: ${freshUrls.length} → ${budget} URLs for ${site.siteName}`);
  ...
  for (const url of freshUrls) {
    try {
      ...
      if (consecutiveFailures >= CONSECUTIVE_FAILURE_THRESHOLD) {
        await sendAlert(`[${site.siteName}] ${CONSECUTIVE_FAILURE_THRESHOLD} consecutive fetch failures`);
        consecutiveFailures = 0;
      }
    }
  }
  ...
  if (insertCount > 0) {
    await sendAlert(`[${site.siteName}] ${insertCount} new topic(s) discovered`);
  }
}
