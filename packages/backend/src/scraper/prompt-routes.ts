...
export async function registerPromptRoutes(app: FastifyInstance): Promise<void> {
  ...
  // 创建新模板
  app.post<{ Body: PromptTemplateCreate }>('/api/v1/prompts', async (request, reply) => {
  ...
  // 获取单个模板
  app.get<{ Params: PromptIdParams }>('/api/v1/prompts/:id', async (request, reply) => {
  ...
  // 更新模板
  app.put<{ Params: PromptIdParams; Body: PromptTemplateUpdate }>('/api/v1/prompts/:id', async (request, reply) => {
    ...
    const updated: PromptTemplate = {
      ...
      ...(body.fewShotExamples !== undefined ? { fewShotExamples: body.fewShotExamples } : {}),
  ...
  // 删除模板
  app.delete<{ Params: PromptIdParams }>('/api/v1/prompts/:id', async (request, reply) => {
    const prompt = await loadPrompt(request.params.id);
    if (!prompt) return err(reply, 404, 'Prompt not found');
