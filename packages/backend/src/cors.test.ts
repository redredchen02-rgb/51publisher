...
describe('CORS origin filtering', () => {
  it('echoes back a configured chrome-extension origin', async () => {
    ...
    expect(res.headers['access-control-allow-origin']).toBe('chrome-extension://abc123deadbeef');
    await app.close();
  });
  ...
  it('allows both dev and prod extension IDs when comma-separated', async () => {
    const app = buildApp('chrome-extension://devid111,chrome-extension://prodid222');
    await app.ready();
    for (const origin of ['chrome-extension://devid111', 'chrome-extension://prodid222']) {
      const res = await app.inject({
        method: 'GET',
        url: '/ping',
        headers: { origin },
      });
      expect(res.headers['access-control-allow-origin'], `origin ${origin}`).toBe(origin);
    }
    await app.close();
  });
  ...
  it('denies all origins when CORS_ORIGIN is only whitespace or wildcard', async () => {
    for (const bad of ['*', '  *  ', ' , ']) {
      ...
      expect(res.headers['access-control-allow-origin'], `bad value "${bad}"`).toBeUndefined();
      await app.close();
    }
  });
});
