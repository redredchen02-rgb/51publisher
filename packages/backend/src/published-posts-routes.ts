  ...
  app.post<{ Body: PublishedPostBody }>('/api/v1/published-posts', async (request, reply) => {
    ...
    const result = await pendingWriteQueue.enqueue(() => {
      ...
      const existing = publish_url
        ? (db.prepare('SELECT id FROM published_posts WHERE publish_url = ?').get(publish_url) as { id: string } | null)
        : (db.prepare('SELECT id FROM published_posts WHERE id = ?').get(id) as { id: string } | null);
      ...
      const row = db.prepare('SELECT * FROM published_posts WHERE id = ?').get(upsertId) as Record<string, unknown>;
