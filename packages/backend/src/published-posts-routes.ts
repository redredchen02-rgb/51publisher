...
export async function registerPublishedPostsRoutes(
	app: FastifyInstance,
): Promise<void> {
		...
		async (request, reply) => {
			...
			const result = await pendingWriteQueue.enqueue(() => {
				...
				const existing = publish_url
					? (db
							.prepare("SELECT id FROM published_posts WHERE publish_url = ?")
							.get(publish_url) as { id: string } | null)
					: (db
							.prepare("SELECT id FROM published_posts WHERE id = ?")
							.get(id) as { id: string } | null);

				// When publish_url matched an existing record, reuse that record's id
				// so ON CONFLICT(id) correctly upserts instead of inserting a duplicate row.
				const upsertId = existing?.id ?? id;
	...
	app.get<{ Querystring: { sourceTitle?: string } }>(
		"/api/v1/published-posts",
		async (request) => {
			const db = getDb();
			...
			const rows = sourceTitle
				? (db
						.prepare(
							"SELECT * FROM published_posts WHERE source_title = ? ORDER BY created_at DESC",
						)
						.all(sourceTitle) as Record<string, unknown>[])
				: (db
						.prepare("SELECT * FROM published_posts ORDER BY created_at DESC")
