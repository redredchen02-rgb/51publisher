...
import { DB_PATH } from './scraper/pending-db.js';
...
describe('GET /api/v1/published-posts', () => {
  ...
  it('返回所有记录', async () => {
    await app.inject({
      method: 'POST',
      url: '/api/v1/published-posts',
      payload: { id: 'a', publish_url: 'https://example.com/a', source_title: 'A' },
    });
    await app.inject({
      method: 'POST',
      url: '/api/v1/published-posts',
      payload: { id: 'b', publish_url: 'https://example.com/b', source_title: 'B' },
    });
  ...
  it('?sourceTitle=X → 只返回匹配条目', async () => {
    await app.inject({
      method: 'POST',
      url: '/api/v1/published-posts',
      payload: { id: 'x1', publish_url: 'https://example.com/x1', source_title: 'X作品' },
    });
    await app.inject({
      method: 'POST',
      url: '/api/v1/published-posts',
      payload: { id: 'y1', publish_url: 'https://example.com/y1', source_title: 'Y作品' },
    });
