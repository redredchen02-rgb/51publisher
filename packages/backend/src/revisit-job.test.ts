...
vi.mock('./scraper/pending-db.js', () => ({
  getDb: vi.fn(() => mockDb),
  pendingWriteQueue: { enqueue: vi.fn((fn: () => unknown) => Promise.resolve(fn())) },
}));
...
function startAndGetCallbacks(): [() => Promise<void>, () => Promise<void>] {
  ...
  return [calls[0]![1] as () => Promise<void>, calls[1]![1] as () => Promise<void>];
}
...
beforeEach(() => {
  ...
  vi.mocked(pendingWriteQueue).enqueue.mockImplementation((fn: () => unknown) => Promise.resolve(fn()));
...
describe('immediate sweep — HTTP 404 → outcome "failed" + alert', () => {
  it('sets outcome to "failed" and sends alert with title + URL', async () => {
    ...
    expect(mockRun).toHaveBeenCalledWith('failed', expect.any(String), 'post-1');
  });
});
...
describe('scheme gate — file:// URL → skip without fetch', () => {
  it('skips rows with non-http(s) publish_url', async () => {
    mockAll.mockReturnValueOnce([makeRow({ publish_url: 'file:///etc/passwd' })]);
