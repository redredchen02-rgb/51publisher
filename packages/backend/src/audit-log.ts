...
export type AuthResult = 'success' | 'invalid_password' | 'rate_limited' | 'not_configured';

export function auditLogin(result: AuthResult, ip: string): void {
  try {
    mkdirSync(LOG_DIR, { recursive: true });
    appendFileSync(AUDIT_LOG_PATH, `${JSON.stringify({ t: new Date().toISOString(), ip, result })}\n`);
  } catch {
