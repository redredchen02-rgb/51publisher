...
describe('sendAlert', () => {
  ...
  beforeEach(() => {
    cleanTgEnv();
    vi.mocked(assertUrlSafe).mockResolvedValue(new URL('https://api.telegram.org'));
  ...
  it('calls fetch with correct URL and body when TG_ENABLED=true', async () => {
    setEnv({ TG_ENABLED: 'true', TG_BOT_TOKEN: VALID_TOKEN, TG_CHAT_ID: VALID_CHAT_ID });
  ...
  it('does not call fetch when TG_ENABLED=false', async () => {
    setEnv({ TG_ENABLED: 'false', TG_BOT_TOKEN: VALID_TOKEN, TG_CHAT_ID: VALID_CHAT_ID });
    await sendAlert('hello');
    expect(fetchSpy).not.toHaveBeenCalled();
  });
  ...
  it('redacts admin domain from message and calls console.warn', async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('[telegram] admin domain redacted'));
  });
  ...
  it('does not redact when message contains no admin domain', async () => {
    ...
    expect(warnSpy).not.toHaveBeenCalledWith(expect.stringContaining('redacted'));
  });
});
...
describe('checkEnv: TG guard', () => {
  ...
  it('TG_ENABLED=true with empty TG_BOT_TOKEN → error', () => {
    const errors = checkEnv(goodEnv({ TG_ENABLED: 'true', TG_BOT_TOKEN: '', TG_CHAT_ID: VALID_CHAT_ID }));
    expect(errors.some((e) => e.includes('TG_BOT_TOKEN') && e.includes('missing'))).toBe(true);
  });

  it('TG_ENABLED=true with wrong format TG_BOT_TOKEN → error', () => {
    const errors = checkEnv(goodEnv({ TG_ENABLED: 'true', TG_BOT_TOKEN: 'bad-token', TG_CHAT_ID: VALID_CHAT_ID }));
    expect(errors.some((e) => e.includes('TG_BOT_TOKEN') && e.includes('format'))).toBe(true);
  });

  it('TG_ENABLED=true with valid token but empty TG_CHAT_ID → error', () => {
    const errors = checkEnv(goodEnv({ TG_ENABLED: 'true', TG_BOT_TOKEN: VALID_TOKEN, TG_CHAT_ID: '' }));
    expect(errors.some((e) => e.includes('TG_CHAT_ID') && e.includes('missing'))).toBe(true);
  });

  it('TG_ENABLED=true with valid token and chat ID → no TG errors', () => {
    const errors = checkEnv(goodEnv({ TG_ENABLED: 'true', TG_BOT_TOKEN: VALID_TOKEN, TG_CHAT_ID: VALID_CHAT_ID }));
