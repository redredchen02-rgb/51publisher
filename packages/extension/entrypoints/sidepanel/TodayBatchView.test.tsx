...
function makeTopic(id: string, overrides: Partial<PendingTopic> = {}): PendingTopic {
  return {
    id,
    sourceUrl: `https://example.com/${id}`,
    siteName: 'test-site',
    title: `选题 ${id}`,
    facts: { 作品名: '测试作品', 集数: '01', 制作: '', 漢化: '', 無修: '', 题材: '', 简介: '' },
    confidence: 0.9,
    status: 'pending',
...
describe('U8 — TodayBatchView', () => {
  ...
  it('resolveAdminTabId 返回 null 时显示 tab 错误', async () => {
    ...
    render(<TodayBatchView onBack={vi.fn()} />);
    await waitFor(() => expect(screen.getByRole('alert').textContent).toContain('未找到后台发帖页'));
  });
  ...
  it('点击「一键备稿」时以正确的 topicIds 调用 runBatch', async () => {
    ...
    const [calledTopics, calledTabId, calledFacts, , , calledTopicIds] = (runBatch as ReturnType<typeof vi.fn>).mock
      .calls[0] as [string[], number, Record<string, string>[], undefined, undefined, string[]];
  ...
  it('遵守 dailyBatchSize:只发送 N 条选题', async () => {
    ...
    await waitFor(() => expect(runBatch).toHaveBeenCalled());
    const [calledTopics] = (runBatch as ReturnType<typeof vi.fn>).mock.calls[0] as [string[]];
