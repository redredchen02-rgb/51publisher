...
describe('R1 — inline fact editing', () => {
  it('展开选题后渲染事实输入框', async () => {
    ...
    render(<PendingTopicsView onBack={vi.fn()} onBatchStarted={vi.fn()} onError={vi.fn()} />);
  ...
  it('编辑 facts 后批准 → PATCH 含更新后的值', async () => {
    ...
    fireEvent.click(screen.getByText('详情'));
    await waitFor(() => screen.getByDisplayValue('测试作品'));
    fireEvent.change(screen.getByDisplayValue('测试作品'), { target: { value: '改后作品名' } });
    ...
    await waitFor(() => {
      expect(patchPendingTopic).toHaveBeenCalledWith(
        't1',
        expect.objectContaining({ facts: expect.objectContaining({ 作品名: '改后作品名' }) }),
      );
    });
  });
});
...
describe('R2 — cover thumbnail', () => {
  it('有 coverImageUrl 时展开后显示 img', async () => {
    ...
    await waitFor(() => screen.getByText('选题 t1'));
    fireEvent.click(screen.getByText('详情'));
    const img = document.querySelector('img[alt="封面"]') as HTMLImageElement | null;
