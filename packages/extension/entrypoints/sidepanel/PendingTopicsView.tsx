...
const FACTS_KEYS = ['作品名', '集数', '制作', '漢化', '無修', '题材', '简介'] as const;
...
export function PendingTopicsView({ onBack, onBatchStarted, onError }: Props) {
  ...
  const [localFacts, setLocalFacts] = useState<Record<string, Record<string, string>>>({});
  ...
  async function handleRejectSelected() {
    ...
    try {
      const selectedTopics = topics.filter((t) => selected.has(t.id));
      await Promise.all(selectedTopics.map((t) => updatePendingStatus(t.id, 'rejected', 'manual reject')));
      setSelected(new Set());
      await refresh();
    } catch {
      onError('操作失败,请重试。');
    ...
    <main style={{ fontFamily: 'system-ui, sans-serif', padding: 12, fontSize: 14 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          <button onClick={onBack} style={{ ...btn, background: '#f0f0f0', color: '#333', padding: '4px 10px' }}>
            ← 返回
          </button>
        </div>
      </div>
      ...
      {!loading && topics.length === 0 && (
        <div style={{ color: '#888', fontSize: 13, marginTop: 16, textAlign: 'center' }}>
          暂无待审核选题。
      ...
      {topics.length > 0 && (
        <>
          ...
          <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
            {topics.map((t) => (
              <li key={t.id} style={{ border: '1px solid #f0f0f0', borderRadius: 4, marginBottom: 4 }}>
                <div style={{ display: 'flex', alignItems: 'center', padding: '6px 8px' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: '#888', marginTop: 1 }}>
                      {t.siteName} · 置信度:{Math.round(t.confidence * 100)}% · {t.sourceUrl.slice(0, 60)}
                    </div>
                  </div>
                  <button
                  ...
                  <div style={{ padding: '6px 10px', fontSize: 12, borderTop: '1px solid #f5f5f5' }}>
                    {t.coverImageUrl && (
                      <img
                        src={t.coverImageUrl}
                        alt="封面"
                        style={{ maxHeight: 60, marginBottom: 6, objectFit: 'cover', borderRadius: 2 }}
                      />
                    )}
                    <div>
                      ...
                      >
                        {FACTS_KEYS.map((key) => (
                          <React.Fragment key={key}>
                            <label style={{ fontSize: 11, color: '#888', textAlign: 'right' }}>{key}</label>
                            <input
                              type="text"
                              value={(localFacts[t.id] ?? t.facts)[key] ?? ''}
                              onChange={(e) => setFactField(t.id, key, e.target.value)}
                              disabled={busy}
                              style={{
                                fontSize: 11,
                                padding: '1px 4px',
                    ...
                    {t.rawContent?.body && (
                      <div style={{ marginTop: 6, maxHeight: 120, overflow: 'auto', color: '#888', fontSize: 11 }}>
                        <strong>原始内容(前300字):</strong>
                        <div style={{ marginTop: 2 }}>{t.rawContent.body.slice(0, 300)}…</div>
                      </div>
                    )}
                  </div>
                )}
