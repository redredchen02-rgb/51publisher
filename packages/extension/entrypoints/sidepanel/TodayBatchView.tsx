...
export function TodayBatchView({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    void (async () => {
      const [settings, tabId] = await Promise.all([getSettings(), resolveAdminTabId()]);
  ...
  async function handleDailyBatch() {
    ...
    try {
      // 拉取高分待审选题,截取前 N 条。
      const pendingTopics = await fetchPendingTopics({ status: 'pending', sort_by: 'score' });
      ...
      await runBatch(topics, adminTabId, factsList, undefined, undefined, topicIds);

      // 构造轻量展示列表(真实批次状态由 BatchView 展示,此处仅给用户即时反馈)。
      const preview: BatchItem[] = topN.map((t, i) => ({
        id: topicIds[i],
        topic: t.title,
        facts: t.facts ?? {},
        status: 'queued',
