...
function defaultSecurityAlert(event: string, detail?: unknown): void {
	logger.warn("bg", `[SECURITY] ${event}`, { detail: detail ?? "" });
}
...
export function createHandlers(deps: BackgroundHandlerDeps) {
	...
	async function runStartupReset(): Promise<void> {
		try {
			...
			logger.warn("bg", "first-flight startup reset 失败", { err: e });
		}
	}
	...
	async function handleWatchdog(): Promise<void> {
		try {
			...
			logger.warn("bg", "first-flight watchdog 失败", { err: e });
		}
	}
			...
			if (result) {
				...
				if (confirmedTopics.length > 0) {
					deps
						...
						.catch((e) =>
							logger.warn("bg", "addPublishedTopics 写入失败(best-effort)", { err: e }),
						);
				}
			}
			return result;
		} catch (err) {
			logger.error("bg", "发布失败", { err });
			return deps.getBatch();
		}
	}
	...
	async function handleApproveBatch(
		...
		try {
			...
			logger.error("bg", "发布失败(draftOverrides 预存)", { err });
			return deps.getBatch();
		}
		...
		try {
			...
			if (item?.pendingTopicId && rejectionReason) {
				updatePendingStatus(
					...
					"rejected",
					rejectionReason,
				).catch((err) => logger.warn("bg", "updatePendingStatus failed", { err }));
			}
			return next;
		} catch {
	...
	async function handleFirstFlightRehearse(
		...
		try {
			...
			logger.error("bg", "first-flight rehearse 失败", { err });
			return {
				ok: false,
				dryRunGreen: false,
				groundingOk: false,
	...
	async function handleFirstFlightRun(
		...
		try {
			...
			logger.error("bg", "first-flight run 失败", { err });
...
export async function runStartupGeneratingRecovery(
	...
	try {
		...
		logger.warn("bg", "generating recovery scan 失败", { err: e });
	}
}
...
async function runStartupTombstoneScan(): Promise<void> {
	try {
		...
		logger.warn("bg", "tombstone startup scan 失败", { err: e });
	}
}
...
export default defineBackground(() => {
	browser.sidePanel
		...
		.catch((err: unknown) => logger.error("bg", "setPanelBehavior 失败", { err }));
	...
	refreshRemoteMappings()
		...
		.catch((e) => logger.warn("bg", "刷新远程映射失败", { err: e }));
