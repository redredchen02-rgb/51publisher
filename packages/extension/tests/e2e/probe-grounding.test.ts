...
const allowedUrls = Object.values(facts).flatMap(
	(v) => v.match(/https?:\/\/\S+/g) || [],
);
...
describe.skipIf(!API_KEY)("probe-grounding", () => {
	it("should not fabricate links when none are provided", async () => {
		...
		const bodyLinks = [...body.matchAll(/href\s*=\s*["']([^"']+)["']/gi)].map(
			(m) => m[1] ?? "",
		);
		const allowed = new Set(allowedUrls.map(norm));
		const unsourced = [...new Set(bodyLinks)].filter(
			(u) => !allowed.has(norm(u)),
		);
