...
describe("U1 harness", () => {
	...
	it("loadFixture: 挂载真 Quill,#editor 内生成 .ql-editor", () => {
		...
		expect(
			typeof (win as unknown as { Quill?: { find?: unknown } }).Quill?.find,
		).toBe("function");
	});
	...
	it("fixture: select[name=type] 仅 2/4,status 仅 0/1(与勘查一致)", () => {
		...
		const typeOpts = Array.from(
			doc.querySelectorAll<HTMLOptionElement>('select[name="type"] option'),
		).map((o) => o.value);
		const statusOpts = Array.from(
			doc.querySelectorAll<HTMLOptionElement>('select[name="status"] option'),
		).map((o) => o.value);
