...
describe('U1 harness', () => {
  ...
  it('fixture: select[name=type] 仅 2/4,status 仅 0/1(与勘查一致)', () => {
    ...
    const typeOpts = Array.from(doc.querySelectorAll<HTMLOptionElement>('select[name="type"] option')).map(
      (o) => o.value,
    );
    const statusOpts = Array.from(doc.querySelectorAll<HTMLOptionElement>('select[name="status"] option')).map(
      (o) => o.value,
    );
