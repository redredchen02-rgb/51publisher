import type { FieldFillResult, SafetyMode } from "@51guapi/shared";
import type { SlotDiff } from "../core/draft-diff";
...
export function buildRecord(
	...
	const base: Omit<TrajectoryRecord, "hash"> = {
		...
		status: input.status,
		ts: input.ts,
		...(input.publishedAsDraft !== undefined
			? { publishedAsDraft: input.publishedAsDraft }
			: {}),
		...(input.mode !== undefined ? { mode: input.mode } : {}),
		...(input.hasManualEdit !== undefined
			? { hasManualEdit: input.hasManualEdit }
			: {}),
		...(input.llmCostTokens !== undefined
			? { llmCostTokens: input.llmCostTokens }
			: {}),
		...
		...(input.aiReviewTriggered !== undefined
			? { aiReviewTriggered: input.aiReviewTriggered }
			: {}),
		...(input.reviewCostTokens !== undefined
			? { reviewCostTokens: input.reviewCostTokens }
			: {}),
		seq,
	};
