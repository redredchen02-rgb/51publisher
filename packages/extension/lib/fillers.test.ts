...
describe("fillers", () => {
	...
	it("native-select:按 value 选中分类并触发 change", () => {
		...
		expect(res?.status).toBe("filled");
		expect(
			(document.querySelector('select[name="type"]') as HTMLSelectElement)
				.value,
		).toBe("2");
	});
	...
	it("checkbox-multi:按标签文本勾选,回车不参与", () => {
		...
		expect(res?.status).toBe("filled");
		expect(
			(document.querySelector("#tag_901") as HTMLInputElement).checked,
		).toBe(true);
		expect(
			(document.querySelector("#tag_902") as HTMLInputElement).checked,
		).toBe(true);
		expect(
			(document.querySelector("#tag_903") as HTMLInputElement).checked,
		).toBe(false);
	});
...
describe("G3: coverUrl field mapping", () => {
	const coverDef: FieldDefinition = {
...
describe("G4: fillCheckboxMulti substring fallback", () => {
	const tagsDefn: FieldDefinition = {
	...
	function makeCheckboxes(...labels: string[]): void {
		document.body.innerHTML = labels
			.map(
				(l, i) =>
					`<input type="checkbox" name="tags[]" id="ctag${i}" /><label for="ctag${i}">${l}</label>`,
			)
			.join("");
	}
	...
	function checkedLabels(): string[] {
		return Array.from(
			...
			.map(
				(b) =>
					document.querySelector<HTMLLabelElement>(`label[for="${b.id}"]`)
						?.textContent ?? "",
			);
	}
...
describe("U8: fillCheckboxMulti degrade detail messages", () => {
	...
	function makeCheckboxes(...labels: string[]) {
		document.body.innerHTML = labels
			.map(
				(l, i) =>
					`<input type="checkbox" name="tags[]" id="utag${i}" /><label for="utag${i}">${l}</label>`,
			)
			.join("");
	}
