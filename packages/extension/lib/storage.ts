...
import type { ContentDraft, SafetyMode, Settings, FewShotPair } from '@51publisher/shared';
...
export async function saveDryRunReport(report: import('@51publisher/shared').DryRunReport): Promise<void> {
  await storage.setItem(DRY_RUN_REPORT_KEY, report);
}

export async function getDryRunReport(): Promise<import('@51publisher/shared').DryRunReport | null> {
