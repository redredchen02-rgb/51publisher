...
import { assembleDraft, sanitizeToPlainText, PLACEHOLDER, type DraftSlots } from '@51publisher/shared';
import { factUrls, type FactsBlock } from '@51publisher/shared';
import { verifyLinks, hasUnsourcedLink } from './link-source';
...
describe('sanitizeToPlainText', () => {
  ...
  it('裸 URL → 【待补】(模型不得自造连结)', () => {
    expect(sanitizeToPlainText('点这里 https://evil.com/x 看')).toBe(`点这里 ${PLACEHOLDER} 看`);
    expect(sanitizeToPlainText('www.evil.com 走起')).toBe(`${PLACEHOLDER} 走起`);
  });
  it('空输入安全', () => {
...
describe('assembleDraft — 散文夹连结/HTML(防注入)', () => {
  it('散文里的 <a>/裸 URL 被剥,绝不进 body 成链', () => {
    const out = assembleDraft(
      slots({ intro: '看这个 <a href="https://fake.com">点我</a> 还有 https://other.com/x' }),
      FULL,
    );
  ...
  it('【不变量】即便散文全是别的域名,verifyLinks 仍无 unsourced', () => {
    const out = assembleDraft(slots({ intro: 'https://a.com https://b.com', highlights: 'www.c.com' }), FULL);
...
describe('assembleDraft — XSS 注入散文', () => {
  it('<script>/onerror 不进 body', () => {
    const out = assembleDraft(
      slots({ intro: '<script>alert(1)</script>正文', highlights: '<img src=x onerror=alert(2)>看点' }),
      FULL,
    );
...
describe('assembleDraft — 连结字段含额外文本', () => {
  it('字段里抽 URL 作 href,与 factUrls 比对一致', () => {
    const out = assembleDraft(slots(), { 作品名: 'X', 漢化: '汉化组:https://h.com/p 已完结' });
    expect(out.body).toContain('<a href="https://h.com/p">');
    expect(hasUnsourcedLink(verifyLinks(out.body, factUrls({ 漢化: '汉化组:https://h.com/p 已完结' })))).toBe(false);
  });
});
