...
const skip = (field: string, note: string): FieldFillResult => ({
	field,
	status: "skipped",
	note,
});
const degrade = (field: string, note: string): FieldFillResult => ({
	field,
	status: "degraded",
	note,
});
...
function labelTextFor(box: HTMLInputElement, doc: Document): string {
	...
	if (next && next.tagName === "LABEL" && next.textContent)
	...
	for (const tag of values) {
		...
		const box =
			...
			(() => {
				...
				for (const [k, b] of substringMap)
					if (k.includes(lc) || lc.includes(k)) return b;
				return undefined;
			})();
		if (box) {
			if (!box.checked) {
