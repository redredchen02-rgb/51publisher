 ...
 * pattern 非法 → false。
 */
export function labelBoundaryMatch(
	rawHost: string,
	rawPattern: string,
): boolean {
...
export function canSubmit(input: GateInput): boolean {
	...
	if (
		!Array.isArray(input.authorizedHosts) ||
		input.authorizedHosts.length === 0
	)
		return false;
	return input.authorizedHosts.some((pattern) =>
		labelBoundaryMatch(input.host, pattern),
	);
}
