...
const FORM_HTML = '<input name="title"><select name="type"></select><div id="editor"></div>';
...
describe('resolveFrameForSelector', () => {
  ...
  it('多层嵌套 iframe → 递归下钻查找', () => {
    ...
    Object.defineProperty(ifr2, 'contentDocument', { get: () => idoc2, configurable: true });
    Object.defineProperty(ifr2, 'contentWindow', { get: () => win2, configurable: true });
    doc1.body.appendChild(ifr2);

    const r = resolveFrameForSelector(document, '#deep-editor', { Quill: 'top' });
