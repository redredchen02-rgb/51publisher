...
describe("trajectory", () => {
	...
	describe("篡改可检", () => {
		...
		it("删中间一条(seq 断裂)→ verify 失败", () => {
			...
			for (const id of ["a", "b", "c"])
				list = appendRecord(list, input({ id })).list;
	...
	it("rollbackTargets:仅 confirmed + 有 URL", () => {
		...
		list = appendRecord(
			list,
			input({ id: "b", status: "error", publishUrl: undefined }),
		).list;
		list = appendRecord(
			list,
			input({ id: "c", status: "publish-confirmed" }),
		).list; // 无 URL
		...
		it("携带度量字段后 hash 链仍可验证", () => {
			...
			list = appendRecord(
				...
				input({
					id: "a",
					mode: "authorized",
					llmCostTokens: { prompt: 10, completion: 5 },
				}),
			).list;
			list = appendRecord(
				list,
				input({
					id: "b",
					slotDiff: { changedSlots: ["title"], totalSlots: 10 },
				}),
			).list;
			expect(verifyTrajectory(list)).toBe(true);
		});
	...
	describe("Phase-3 评审字段携带", () => {
		...
		it("携带 Phase-3 字段后 hash 链仍可验证（新字段不影响 canonical）", () => {
			...
			list = appendRecord(
				list,
				input({ id: "b", aiReviewTriggered: false }),
			).list;
			expect(verifyTrajectory(list)).toBe(true);
		});
		...
		it("旧记录（无 Phase-3 字段）verifyTrajectory 仍返回 true", () => {
			...
			const oldRecord: TrajectoryRecord = buildRecord(
				input({ id: "old" }),
				1,
				"0",
			).record;
