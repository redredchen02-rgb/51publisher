...
import { saveSettings, saveBackendToken, DEFAULT_SETTINGS } from './storage';
...
describe('recordPublishedPost', () => {
  ...
  it('backendUrl 配置但 token 为空 → 不调用 fetch', async () => {
    await saveSettings({ ...DEFAULT_SETTINGS, backendUrl: 'http://localhost:3001' });
  ...
  it('fetch body 正确序列化字段(snake_case)', async () => {
    ...
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response('', { status: 200 }));
    await recordPublishedPost({ ...BASE_RECORD, outcome: 'publish-confirmed', publishUrlSource: 'from_save' });
  ...
  it('publishUrl 缺失 → body publish_url 为 null', async () => {
    ...
    await saveBackendToken('tok');
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response('', { status: 200 }));
    await recordPublishedPost({ id: 'r2', batchItemId: 'item_1', sourceTitle: 'x', publishedAt: '2026-06-11T00:00:00.000Z' });
    const body = JSON.parse((fetchSpy.mock.calls[0]![1] as RequestInit).body as string);
