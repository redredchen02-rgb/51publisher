 ...
 */
export function withBackendSync(
	localSave: (batch: Batch) => Promise<void>,
): (batch: Batch) => Promise<void> {
		...
		try {
			...
			if (!createdRemote) {
				const result = await createRemoteBatch({
					...
					facts: batch.items.map((i) => i.facts) as (
 ...
 * 后端不可达/未登录时回落本地恢复。
 */
export async function tryBackendRecovery(
	batchId: string | null,
): Promise<{ batch?: Batch }> {
	if (!batchId) return {};
	try {
