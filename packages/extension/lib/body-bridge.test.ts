...
function mockMainWorld(ok: boolean, error?: string, degraded?: boolean) {
	const handler = (e: Event) => {
		...
		document.dispatchEvent(
			new CustomEvent<BodyFilledDetail>(EVT_BODY_FILLED, {
				detail: { reqId: detail.reqId, ok, error, degraded },
			}),
		);
	};
...
describe("bodyResultFromOutcome(三态映射)", () => {
	it("ok 且非 degraded → filled,无 note", () => {
		expect(bodyResultFromOutcome({ ok: true })).toEqual({
			field: "body",
			status: "filled",
		});
	});
