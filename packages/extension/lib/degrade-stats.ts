...
export function aggregateDegradeStats(items: BatchItem[]): DegradeStats {
  const withResults = items.filter((it) => it.fillResults && it.fillResults.length > 0);
  const itemsWithAnyDegrade = withResults.filter((it) => it.fillResults!.some((r) => r.status === 'degraded')).length;
  ...
  return { itemsWithAnyDegrade, totalItemsWithResults: withResults.length, topFields };
}
