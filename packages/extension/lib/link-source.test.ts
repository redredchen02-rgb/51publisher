...
import { extractLinks, normalizeUrl, verifyLinks, hasUnsourcedLink } from './link-source';

describe('extractLinks', () => {
  it('pulls hrefs from anchor tags', () => {
    const html = '<p>看 <a href="http://a/1">漢化</a> 和 <a href="https://b/2">無修</a></p>';
...
describe('normalizeUrl', () => {
  it('ignores scheme, lowercases host, strips www and trailing slash', () => {
    expect(normalizeUrl('https://WWW.A.com/x/')).toBe(normalizeUrl('http://a.com/x'));
  });
...
describe('verifyLinks', () => {
  it('marks links found in allowed set as sourced', () => {
    ...
    expect(verifyLinks(html, ['https://a/1'])).toEqual([{ url: 'http://a/1', sourced: true }]);
  });
  ...
  it('marks links NOT in input facts as unsourced (hallucination)', () => {
    ...
    expect(r).toEqual([{ url: 'https://evil-invented.net/g/123', sourced: false }]);
  ...
  it('dedupes normalized-equivalent body links', () => {
    const html = '<a href="https://a.com/x">1</a><a href="http://www.a.com/x/">2</a>';
