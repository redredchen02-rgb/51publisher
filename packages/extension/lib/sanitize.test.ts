...
import { PINNED_DOMPURIFY_VERSION, sanitizeBody } from "./sanitize";

describe("sanitizeBody", () => {
	it("保留正常格式标签", () => {
		const out = sanitizeBody(
			"<p>段落 <strong>粗</strong> <em>斜</em></p><ul><li>项</li></ul>",
		);
	...
	describe("v1 基线硬化(远程资源外泄 / 危险 URI / 版本钉定)", () => {
		...
		it("剥绝对远程 https src(自动加载外泄)", () => {
			const out = sanitizeBody(
				'<img src="https://attacker.example/track.gif?c=1">',
			);
			expect(out).not.toMatch(/attacker/);
		});
		...
		it("拒 data:text/html 与 data:image/svg+xml(mXSS 向量)", () => {
			...
			expect(
				sanitizeBody('<img src="data:image/svg+xml;base64,PHN2Zz4=">'),
			).not.toMatch(/data:/i);
		});
