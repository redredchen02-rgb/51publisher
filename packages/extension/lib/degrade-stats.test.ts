...
describe('aggregateDegradeStats', () => {
  ...
  it('所有字段 filled → 无降级', () => {
    const r = aggregateDegradeStats([
      item('a', [{ field: 'title', status: 'filled' }, { field: 'category', status: 'filled' }]),
    ]);
  ...
  it('一条有降级 → itemsWithAnyDegrade=1', () => {
    const r = aggregateDegradeStats([
      item('a', [{ field: 'category', status: 'degraded' }, { field: 'title', status: 'filled' }]),
  ...
  it('多条同字段降级 → topFields 按次数降序', () => {
    const r = aggregateDegradeStats([
      item('a', [{ field: 'category', status: 'degraded' }, { field: 'tags', status: 'degraded' }]),
      item('b', [{ field: 'category', status: 'degraded' }]),
      item('c', [{ field: 'category', status: 'degraded' }, { field: 'tags', status: 'filled' }]),
    ]);
  ...
  it('topFields 最多取 5 个', () => {
    ...
    const r = aggregateDegradeStats([
      item('a', fields.map((f) => ({ field: f, status: 'degraded' as const }))),
    ]);
