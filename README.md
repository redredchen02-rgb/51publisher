...
- **发布前硬闸（grounding gate）**：以下任一条件触发拦截 →
  - 草稿含【待补】占位符
  - 含无来源裸 URL（description / subtitle）
  - tags 不在设置页白名单（白名单为空时跳过）
  - category 不在合法值集（`2`=漫画 / `4`=动漫）
- **内联事实修改**：gate 拦截后可直接在审核卡片编辑事实字段并触发 LLM 重新生成，无需退出批量视图
