...
/** 收集顶层文档下可访问(同源)的直接子 iframe 的 document/window。 */
function childFrames(rootDoc: Document): ResolvedFrame[] {
  const out: ResolvedFrame[] = [];
  for (const frame of Array.from(rootDoc.querySelectorAll('iframe'))) {
    try {
      const doc = frame.contentDocument;
      const win = frame.contentWindow;
      if (doc && win) out.push({ doc, win });
    } catch {
...
/** 找到含有 `selector` 的 frame(顶层优先,再下钻一层同源 iframe)。找不到 → 退回顶层。 */
...
export function resolveFormFrame(
  ...
  const selectors = Object.values(mapping)
    ...
    .map((d) => d.selector);
  const score = (doc: Document) => selectors.reduce((n, s) => (doc.querySelector(s) ? n + 1 : n), 0);
