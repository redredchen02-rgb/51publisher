...
describe('pasteIntoQuill', () => {
  it('tier ①:window.Quill.find 拿到实例 → dangerouslyPasteHTML 被调用', () => {
    ...
    const win = { Quill: { find: vi.fn((n: Element) => (n === node ? { clipboard: { dangerouslyPasteHTML: paste }, setText } : null)) } };
  ...
  it('未找到编辑器节点 → ok:false', () => {
    const res = pasteIntoQuill('<p>x</p>', '#nope', { Quill: { find: () => null } }, document);
