...
function fillNativeSelect(field: string, def: FieldDefinition, value: string, doc: Document): FieldFillResult {
  ...
  const matched =
    options.find((o) => o.value === value) ?? options.find((o) => o.text.trim() === value.trim());
...
function fillCheckboxMulti(field: string, def: FieldDefinition, values: string[], doc: Document): FieldFillResult {
  ...
  for (const tag of values) {
    const box = byLabel.get(tag.trim());
    if (box) {
      if (!box.checked) {
        box.checked = true;
...
function valueFor(field: keyof FieldMapping, draft: ContentDraft): string | string[] {
  switch (field) {
    ...
    case 'subtitle': return draft.subtitle;
    case 'category': return draft.category;
    case 'description': return draft.description;
    case 'postStatus': return draft.postStatus;
    case 'publishedAt': return draft.publishedAt;
    case 'mediaId': return draft.mediaId;
...
export function fillField(field: keyof FieldMapping, def: FieldDefinition, draft: ContentDraft, doc: Document): FieldFillResult | null {
