...
describe('U3 核心填充路径(真 Quill)', () => {
  ...
  async function runFill(draft: ContentDraft) {
    ...
    const bodyOutcome = await requestBodyFill(sanitizeBody(draft.body), '#editor', 3000, doc);
    await microtask();
  ...
  it('文本字段:title/subtitle/description/media_id/published_at 正确写入', async () => {
    ...
    expect(doc.querySelector<HTMLInputElement>('input[name="title"]')!.value).toBe(draft.title);
    expect(doc.querySelector<HTMLInputElement>('input[name="subtitle"]')!.value).toBe(draft.subtitle);
    expect(doc.querySelector<HTMLTextAreaElement>('textarea[name="description"]')!.value).toBe(draft.description);
    expect(doc.querySelector<HTMLInputElement>('input[name="media_id"]')!.value).toBe(draft.mediaId);
    expect(doc.querySelector<HTMLInputElement>('input[name="published_at"]')!.value).toBe(draft.publishedAt);
  });

  it('下拉:type 选中 2、status 选中 1', async () => {
    const { doc } = await runFill(makeDraft());
    expect(doc.querySelector<HTMLSelectElement>('select[name="type"]')!.value).toBe('2');
    expect(doc.querySelector<HTMLSelectElement>('select[name="status"]')!.value).toBe('1');
  });

  it('标签:draft.tags 对应 checkbox 勾选,未列出的保持未选', async () => {
    const { doc } = await runFill(makeDraft({ tags: ['奇幻', '校園'] }));
    const checked = Array.from(doc.querySelectorAll<HTMLInputElement>('input[name="tags[]"]'))
      .filter((b) => b.checked)
      .map((b) => b.id);
  ...
  it('消毒:正文里的 <script> / onerror 不会进 .ql-editor', async () => {
    const draft = makeDraft({ body: '<p>安全</p><script>alert(1)</script><img src=x onerror="alert(1)">' });
    const { doc } = await runFill(draft);
    const editor = doc.querySelector<HTMLElement>('#editor .ql-editor')!;
...
describe('U4 降级路径(window.Quill 不可用)', () => {
  ...
  it('无 window.Quill → tier② 兜底写入,outcome.degraded:true,.ql-editor 含正文', async () => {
    ...
    uninstall = installBodyResponder(doc, win, doc);
    const draft = makeDraft();
    const outcome = await requestBodyFill(sanitizeBody(draft.body), '#editor', 3000, doc);
    await microtask();
  ...
  it('降级路径仍零提交', async () => {
    const { document: doc, window: win, form, publishButton } = loadFixture({ withQuill: false });
    uninstall = installBodyResponder(doc, win, doc);
    const spy = installSubmitSpy(form, publishButton);
    const draft = makeDraft();
