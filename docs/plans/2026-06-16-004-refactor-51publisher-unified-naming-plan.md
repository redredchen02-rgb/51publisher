---
title: "refactor: Unify package naming to @51publisher/* scope + upgrade extension deps"
type: refactor
status: active
date: 2026-06-16
deepened: 2026-06-16
...
将三个命名风格不一的包（`publisher-monorepo`、`publisher-fill-assistant`、`publisher-backend`）统一到 `@51publisher/*` scope，与 `@51publisher/shared` 保持一致；同时重刷 lockfile（可选随附 vitest patch 升级）。

## Problem Frame

当前状态：
- `@51publisher/shared` — 已有 scope，格式正确
...
- **不升** 带破坏性变更的 major 版本
- **不动** `@51publisher/shared` 的名称（已正确）
- docs/plans/archive 里的旧计划文件**不修改**（历史记录）

## Requirements Trace

- R1. 四个包名称全部使用 `@51publisher/*` scope
...
- **根包名**：`publisher-monorepo` → `@51publisher/monorepo`。根包是 private workspace root，改名原因是统一全仓库命名风格（虽然根包很少被 `--filter` 引用，但一致性减少未来困惑）。
- **扩展包名**：`publisher-fill-assistant` → `@51publisher/extension`。这是 `--filter` 引用最多的包，改名后需全局替换。
- **后端包名**：`publisher-backend` → `@51publisher/backend`。同上。
...
- **wxt zip 产物名**：wxt 将 `@` 和 `/` 替换为 `-`，`@51publisher/extension` 的 zip 产物名为 `51publisher-extension-<version>.zip`（leading `@` 被移除）。**在 Unit 3 执行前须本地 `pnpm zip` 确认**，再更新 release.yml glob。

## Implementation Units

- [ ] **Unit 1: 重命名三个包的 package.json name 字段 + 根 scripts**

**Goal:** 把 `publisher-monorepo`、`publisher-fill-assistant`、`publisher-backend` 统一改为 `@51publisher/*`；同时更新根 package.json 的 scripts 区段
...
**Files:**
- Modify: `package.json`（根）— `name` → `@51publisher/monorepo`；`scripts` 中四条 filter 引用：`publisher-fill-assistant` → `@51publisher/extension`，`publisher-backend` → `@51publisher/backend`
- Modify: `packages/extension/package.json`（name → `@51publisher/extension`）
- Modify: `packages/backend/package.json`（name → `@51publisher/backend`）

**Approach:**
- 根 package.json 需改两处：`"name"` 字段 + `scripts.dev:extension`、`scripts.dev:backend`、`scripts.build:extension`、`scripts.build:backend` 四条命令
- `@51publisher/shared` 已正确，不动

**Test scenarios:**
- Happy path: `cat package.json | grep '"name"'` 输出 `@51publisher/monorepo`
- Happy path: `cat packages/extension/package.json | grep '"name"'` 输出 `@51publisher/extension`
- Happy path: `cat packages/backend/package.json | grep '"name"'` 输出 `@51publisher/backend`
- Happy path: `pnpm --filter "@51publisher/extension" build` 执行成功（验证 scoped filter 语法可用）
...
**Approach:**
- 全局替换：`publisher-fill-assistant` → `@51publisher/extension`，`publisher-backend` → `@51publisher/backend`
- shell 中含 `@` 的 filter 需加引号：`pnpm --filter "@51publisher/extension" build`
- YAML 中也需要引号：`run: pnpm --filter "@51publisher/extension" test`
...
- ci.yml 需更新三处（全部 `--filter publisher-*`）：
  1. Build backend：`--filter publisher-backend` → `--filter "@51publisher/backend"`
  2. Build extension：`--filter publisher-fill-assistant` → `--filter "@51publisher/extension"`
  3. E2E 步骤：`--filter publisher-fill-assistant test:e2e` → `--filter "@51publisher/extension" test:e2e`
- release.yml 需更新两处：
  1. Zip 命令：`--filter publisher-fill-assistant zip` → `--filter "@51publisher/extension" zip`
  2. Artifact cp glob：`publisher-fill-assistant-*.zip` → `51publisher-extension-*.zip`（wxt 将 `@` 去掉，`/` 换 `-`）
- **执行前必须**：在本地运行 `pnpm --filter "@51publisher/extension" zip` 确认实际产物文件名，再更新 glob

**Test scenarios:**
- Pre-flight: 本地 `pnpm --filter "@51publisher/extension" zip` 成功，确认产物名为 `51publisher-extension-<version>.zip`
...
- archive 下的历史计划**不改**
- `pnpm -r` / `pnpm --filter @51publisher/shared` 等已正确的不动
...
| `@` 在 shell --filter 中被 glob 展开 | 所有 shell/YAML filter 参数加引号：`"@51publisher/extension"` |
...
- 现有正确示范：`packages/shared/package.json` → `"name": "@51publisher/shared"`
