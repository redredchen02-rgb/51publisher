...
async function evaluateGate(tabId: number): Promise<GateDecision> {
	const [mode, authorizedHosts] = await Promise.all([
		getSafetyMode(),
		getAuthorizedHosts(),
	]);
	...
	try {
		return await orchestratePublish({
			...
			isAlreadyDispatched: async () =>
				(await storage.getItem(markerKey(tabId))) === "publish-dispatched",
			writeDispatched: () =>
				storage.setItem(markerKey(tabId), "publish-dispatched"),
			sendGrant: async () => {
				try {
	...
	try {
		...
		for (const item of batch.items) {
			...
			const gen = await generateDraft(
				buildPrompt(settings.promptTemplate, item.topic),
				{ settings, apiKey },
			);
	...
	try {
		...
		for (const snapshot of batch.items) {
			...
			const result = await orchestratePublish({
				evaluateGate: () => evaluateGate(tabId),
				isAlreadyDispatched: async () =>
					itemStatus(batch, item.id) === "publish-dispatched",
				...
				sendGrant: async () => {
					try {
						return asPublishResult(
							await browser.tabs.sendMessage(tabId, { type: "PUBLISH_GRANT" }),
						);
					} catch {
						return { ok: false, dryRun: false, error: "content-unreachable" };
					}
			...
			// 轨迹:全档位落档(R6a)。
			{
				...
				if (snapshotDropped)
					console.warn("[bg] 轨迹快照含机密被丢弃(record 已落,无快照)");
