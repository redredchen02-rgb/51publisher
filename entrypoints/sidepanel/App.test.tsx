...
const draft: ContentDraft = {
  id: 'd1', title: 'AI 标题', subtitle: '', category: '2', coverImageUrl: '',
  body: '<p>正文</p>', tags: ['奇幻'], description: '', postStatus: '1',
  publishedAt: '', mediaId: '', status: 'draft', createdAt: '2026-06-03T00:00:00.000Z',
};
...
describe('App', () => {
  ...
  it('填充 → 显示结果面板;有问题字段进入 partial', async () => {
    requestGenerate.mockResolvedValue({ ok: true, draft });
    requestFill.mockResolvedValue({ ok: true, results: [{ field: 'title', status: 'filled' }, { field: 'body', status: 'degraded', note: '需手动' }] });
    render(<App />);
    fireEvent.change(screen.getByPlaceholderText(/输入选题/), { target: { value: '某新番' } });
    fireEvent.click(screen.getByText('生成草稿'));
