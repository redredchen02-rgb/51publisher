...
export function HistoryPanel() {
  ...
  const rollbackSet = new Set(rollbackTargets([...records].reverse()).map((r) => r.id));
  const visible = records.slice(0, page * PAGE_SIZE);
  const hasMore = records.length > page * PAGE_SIZE;

  return (
    <section style={{ paddingTop: 10 }}>
      <div style={{ fontSize: 12, marginBottom: 6, color: intact ? '#389e0d' : '#cf1322' }}>
        {intact ? '✓ 链完整' : '⚠ 链异常(疑被篡改)'}
        <span style={{ color: '#888', marginLeft: 8 }}>共 {records.length} 条</span>
      </div>
      <ul style={{ listStyle: 'none', padding: 0, margin: 0, fontSize: 12 }}>
        {visible.map((r) => (
          <li key={r.id} style={{ marginBottom: 6, padding: '5px 8px', background: '#fafafa', borderRadius: 4 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '60%' }}>
                「{r.topic}」
              </span>
              <span style={{ fontSize: 11, color: '#888' }}>{new Date(r.ts).toLocaleString()}</span>
            </div>
            ...
            {r.fields && r.fields.some((f) => f.status === 'degraded') && (
              <div style={{ fontSize: 11, color: '#d46b08', marginTop: 2 }}>
                ⚠ {r.fields.filter((f) => f.status === 'degraded').length} 个字段降级
              </div>
            )}
          </li>
        ))}
      ...
      {hasMore && (
        <button
          ...
          style={{ marginTop: 6, fontSize: 12, padding: '3px 10px', border: '1px solid #d9d9d9', borderRadius: 4, cursor: 'pointer', background: '#fff' }}
        >
          加载更多
        </button>
      )}
