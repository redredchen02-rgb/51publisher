...
function pair(input = '', output = ''): FewShotPair { return { input, output }; }
...
describe('FewShotPairEditor', () => {
  ...
  it('上移第一张 → 按钮 disabled', () => {
    render(<FewShotPairEditor pairs={[pair('A', 'B'), pair('C', 'D')]} onChange={vi.fn()} />);
  ...
  it('导入 banner 存在时显示，点击后调 onImport', () => {
    ...
    render(<FewShotPairEditor pairs={[]} onChange={vi.fn()} importBanner="检测到旧格式" onImport={onImport} />);
